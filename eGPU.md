...
If you have a workstation, we have a [GC-Titan Ridge card]([https://www.gigabyte.com/Motherboard/GC-TITAN-RIDGE-rev-10#kf) available, which you can use if you have a free 4 PCIe lane slot. Note that the GC-Titan Ridge Installation Guide also requires to be connected internally with both a double power cable a, a USB-B to a free F_USB connector and a USB-C to a free THB_C connector on the motherboard. There is [a rumor](https://egpu.io/forums/thunderbolt-enclosures/list-of-intel-titan-ridge-thunderbolt-3-devices/paged/3/) that the card can also work without those connections, but that has still to be confirmed.
